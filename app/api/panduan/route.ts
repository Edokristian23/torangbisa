...
export async function GET() {
  try {
    ...
    return NextResponse.json({ message: 'Gagal memuat dokumen panduan.' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user?.isActive || !ADMIN_ROLES.has(user.role)) {
      return NextResponse.json({ message: 'Hanya Admin BPKP yang dapat menambahkan panduan.' }, { status: 403 });
    }
    ...
    if (!name) {
      return NextResponse.json({ message: 'Nama dokumen wajib diisi sebelum upload.' }, { status: 400 });
    }

    if (!(file instanceof File)) {
      return NextResponse.json({ message: 'File dokumen wajib diupload.' }, { status: 400 });
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ message: 'Ukuran file maksimal 15MB.' }, { status: 400 });
    }
    ...
    await createAuditLog({
      ...
      entityId: created.id,
      metadata: { name, originalName: file.name, fileSize: file.size },
    });

    return NextResponse.json({ message: 'Dokumen panduan berhasil ditambahkan.', document: created }, { status: 201 });
  } catch (error) {
    console.error('[POST_PANDUAN]', error);
    return NextResponse.json({ message: 'Gagal menambahkan dokumen panduan.' }, { status: 500 });
  }
}
