...
export default function UserPage() {
  ...
  const toggleActive = async (user: UserItem) =>
    patchUser(user.id, { isActive: !user.isActive }, "Status user berhasil diperbarui.");
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] min-h-0 flex-col gap-6 overflow-hidden">
      <div className="flex shrink-0 flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">User Management</h2>
          <p className="text-sm text-slate-500">
            Kelola multi-role, reset password, dan penguncian akun untuk BPKP dan BLUD.
          </p>
        </div>
      ...
      {(message || error) && (
        <div
          className={`shrink-0 rounded-2xl px-4 py-3 text-sm ${
            error
      ...
      {showCreate && (
        <div className="shrink-0 grid gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2 xl:grid-cols-3">
          <input
            value={form.username}
          ...
          <select
            ...
            onChange={(e) =>
              setForm((prev) => ({
                ...prev,
                role: e.target.value,
                bludId: ["BLUD_ADMIN", "BLUD_OPERATOR"].includes(e.target.value) ? prev.bludId : "",
              }))
            }
            className="rounded-2xl border px-4 py-3 text-sm"
          >
          ...
          <select
            value={form.bludId}
            onChange={(e) => setForm((prev) => ({ ...prev, bludId: e.target.value }))}
            disabled={!["BLUD_ADMIN", "BLUD_OPERATOR"].includes(form.role)}
            className="rounded-2xl border px-4 py-3 text-sm disabled:bg-slate-100"
          >
            <option value="">Pilih BLUD</option>
            {bluds.map((blud) => (
              <option key={blud.id} value={blud.id}>
                {blud.code} - {blud.name}
              </option>
            ))}
          </select>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        {loading ? (
                ...
                {users.map((user) => (
                  <tr key={user.id} className="border-b align-top last:border-b-0">
                    ...
                    <td className="px-3 py-4 text-xs text-slate-600">
                      {user.bludCode ? `${user.bludCode} - ${user.bludName}` : "-"}
                    </td>
                    <td className="px-3 py-4 text-xs text-slate-600">
                      <p>
                        Status:{" "}
                        <span className={user.isActive ? "text-emerald-700" : "text-red-700"}>
                    ...
                    <td className="px-3 py-4">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700 hover:bg-amber-100"
                        >
                        ...
                        <button
                          onClick={() =>
                            void patchUser(
                              ...
                              "Flag ganti password diperbarui.",
                            )
                          }
